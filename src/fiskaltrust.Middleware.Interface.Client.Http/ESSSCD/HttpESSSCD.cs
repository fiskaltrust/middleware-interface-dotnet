...
{
    internal class HttpESSSCD : IESSSCD
    {
        private readonly HttpClient _httpClient;

        public HttpESSSCD(HttpITSSCDClientOptions options)
        {
            _httpClient = GetClient(options);
        }

        public async Task<ESSSCDInfo> GetInfoAsync() => await ExecuteHttpGetAsync<ESSSCDInfo>("v1", "GetDeviceInfo").ConfigureAwait(false);
